...
import { Shield, Sparkles, CheckCircle2, Rocket, Zap, Bot } from "lucide-react";
...
export function ProjectInterviewV2() {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 max-w-6xl mx-auto p-4">
      ...
      {/* Right - Agent Avatar with Waves */}
      {!state.isFinished && (
        <div className="lg:w-80 flex flex-col items-center justify-start pt-8">
          <div className="relative w-64 h-64 flex items-center justify-center">
            {/* Outer wave rings */}
            <div className="absolute inset-0 rounded-full border-2 border-[#00f2ff]/20 animate-ping" style={{ animationDuration: '3s' }} />
            <div className="absolute inset-4 rounded-full border-2 border-[#00f2ff]/30 animate-ping" style={{ animationDuration: '2.5s', animationDelay: '0.3s' }} />
            <div className="absolute inset-8 rounded-full border-2 border-[#00f2ff]/40 animate-ping" style={{ animationDuration: '2s', animationDelay: '0.6s' }} />
            <div className="absolute inset-12 rounded-full border-2 border-[#00f2ff]/50 animate-ping" style={{ animationDuration: '1.5s', animationDelay: '0.9s' }} />
            
            {/* Main circle background */}
            <div className="absolute w-40 h-40 rounded-full bg-gradient-to-br from-[#00f2ff]/20 to-[#0088aa]/20 border-2 border-[#00f2ff]/40 shadow-[0_0_30px_rgba(0,242,255,0.3)] animate-pulse" />
            
            {/* Agent icon */}
            <div className="relative z-10 w-24 h-24 rounded-full bg-[#00f2ff]/25 border-2 border-[#00f2ff]/60 flex items-center justify-center shadow-[0_0_20px_rgba(0,242,255,0.5)]">
              <Bot className="w-14 h-14 text-[#00f2ff]" />
            </div>

            {/* Rotating ring */}
            <div className="absolute w-48 h-48 rounded-full border border-dashed border-[#00f2ff]/30 animate-spin" style={{ animationDuration: '20s' }} />
            <div className="absolute w-52 h-52 rounded-full border border-dotted border-[#00f2ff]/20 animate-spin" style={{ animationDuration: '30s', animationDirection: 'reverse' }} />
          </div>

          <div className="mt-8 text-center space-y-2">
            <h3 className="text-[#00f2ff] font-bold uppercase tracking-widest text-sm">Project Owner</h3>
            <p className="text-white/40 text-xs">IA Neural Agent v2.0</p>
            <div className="flex items-center justify-center gap-2 text-[10px] text-white/30 uppercase tracking-[0.2em]">
              <Sparkles className="w-3 h-3 animate-pulse" /> Procesando
            </div>
          </div>
        </div>
      )}
